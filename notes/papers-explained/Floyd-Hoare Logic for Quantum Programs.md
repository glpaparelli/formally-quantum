...
Then the partial [[Trace]] of $A$ on $\mathcal{H}$ is defined as $$tr_k(A)=\Sigma_i(I_\mathcal{H}\bigotimes\langle\psi_i|)A(I_\mathcal{H}\bigotimes|\psi_i\rangle)$$which is an operator on $\mathcal{H}$, where $\{\psi_i\}$ is an [[Orthogonal-Orthonormal Basis]] of $\mathcal{K}$. It can be shown that $tr_k(A)$ does not depend on the choice of $\{\psi_i\}$. 
In particular, if $\mathcal{H}_1$ and $\mathcal{H}_2$ are the state spaces of quantum systems $q_1$ and $q_2$, respectively, and the state of their composite system $q_1q_2$ is described by a [[Density Matrix]] $\rho \in \mathcal{D}^{-}(\mathcal{H_1}\bigotimes\mathcal{H_2})$, then $tr_\mathcal{H_2}(\rho)$ is the description for the state of component system  $q_1$. 
...
- $U$ in the statement $\bar{q}:=U\bar{q}$ is a [[Unitary Transformation on QC]] on $\mathcal{H}_\bar{q}$. 
...
- $\rho \in \mathcal{D}^-(\mathcal{H}_\text{all})$ is a [[Density Matrix#Partial Density Operator]] on $\mathcal{H}_\text{all}$ and it is used to indicate the (global) state of quantum variables. 
...
Let $\bar{q} = q_1,\ \dots,\ q_n$ be a quantum register. A [[Linear Transformation]] $\mathcal{A}$ on $\mathcal{H}_\bar{q}$ has a [[Cylinder Extension]] $$A \bigotimes I_{\text{Var} / \{\bar{q}\}}\mathcal{H}_{q} \ \ \ \ \ \ \ \ (1)$$on $\mathcal{H}_{\text{all}}$, where $I_{\text{Var} / \{\bar{q}\}}$ is the identity operator on the Hilbert Space $\bigotimes_{q \in var/{\bar{q}}} \mathcal{H}_q$  
